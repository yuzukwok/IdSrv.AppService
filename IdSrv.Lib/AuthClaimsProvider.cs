...
namespace IdSrv.Lib
{
    ...
    /// </summary>
    public class AuthClaimsProvider : IClaimsProvider
    {
        /// <summary>
        /// The logger
        /// </summary>
        ...
        public AuthClaimsProvider(IUserService users)
        {
            _users = users;
        }
        ...
        public virtual async Task<IEnumerable<Claim>> GetAccessTokenClaimsAsync(ClaimsPrincipal subject, Client client, IEnumerable<Scope> scopes, ValidatedRequest request)
        {
            ...
            //额外的断言
            if (request.Raw.AllKeys.Contains("UA"))
            {
                outputClaims.Add(new Claim("UA", request.Raw.GetValues("UA").FirstOrDefault()));
            }
            if (request.Raw.AllKeys.Contains("ClientId"))
            {
                outputClaims.Add(new Claim("ClientId", request.Raw.GetValues("ClientId").FirstOrDefault()));
            }
